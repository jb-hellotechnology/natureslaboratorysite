<?php
	...
	if($Form->submitted()){
    	...
    	$details = json_decode($specDetails['natures_laboratory_coa_capsules_specDynamicFields'],true);
		...
		$pdf->SetFont('Arial','B',9);$pdf->Cell(60,5,'Product Name: ',0,0);$pdf->SetFont('Arial','',9);$pdf->Cell(100,5,$specDetails['commonname'].' Capsule',0,1);
		...
		$pdf->SetFont('Arial','',9);
		if($specDetails['weight']<>''){$pdf->SetFont('Arial','B',9);$pdf->Cell(60,5,'Capsule Weight:  ',0,0);$pdf->SetFont('Arial','',9);$pdf->Cell(0,5,$specDetails['weight'],0,1);}
		...
		$thisDate = explode("-",$specDetails['dateCreated']);
		...
		$pdf->Output('D',"Natures Laboratory Spec - $specDetails[productCode].pdf");
		exit();
