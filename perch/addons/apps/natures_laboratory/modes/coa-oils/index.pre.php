<?php
    ...
    $NaturesLaboratoryCOA = new Natures_Laboratory_COA_Oils($API);
    ...
    if($Form->submitted()){
		...
		if($specDetails['cas']<>''){$pdf->SetFont('Arial','B',9);$pdf->Cell(60,5,'CAS Number:  ',0,0);$pdf->SetFont('Arial','',9);$pdf->Cell(0,5,$specDetails['cas'],0,1);}
		if($specDetails['einecs']<>''){$pdf->SetFont('Arial','B',9);$pdf->Cell(60,5,'EINECS Number:  ',0,0);$pdf->SetFont('Arial','',9);$pdf->Cell(0,5,$specDetails['einecs'],0,1);}
		if($specDetails['manufacturingMethod']<>''){$pdf->SetFont('Arial','B',9);$pdf->Cell(60,5,'Manufacturing Method:  ',0,0);$pdf->SetFont('Arial','',9);$pdf->Cell(0,5,$specDetails['manufacturingMethod'],0,1);}

		$pdf->SetFont('Arial','B',11);
		$pdf->Cell(0,10,'Product Description',0,1);
		$pdf->SetFont('Arial','',9);
		if($details['appearance']<>''){$pdf->SetFont('Arial','B',9);$pdf->Cell(60,5,'Appearance:  ',0,0);$pdf->SetFont('Arial','',9);$pdf->Cell(0,5,$details['appearance'],0,1);}
		...
		$pdf->WriteHTML("<br><br><br><b>Storage</b><br>Store in cool and dry condition. Keep away from direct sunlight and heat.<br><br><b>Shelf Life</b><br>24 months from manufacture if stored unopened and under storage conditions stated above.<br><br><b>Labels</b><br>Label contains following information:<br>1. Manufacturing company name<br>2. Type of product and product code<br>3. Product Latin and common name<br>4. Pack size<br>5. Best before date<br>6. Contact information<br><br><b>Allergens</b><br><br>");
