...
*/
    ...
    if($Form->submitted()) {
		...
		$notes = wordwrap($productNotes, 42, "<br />");
		...
		while($bag<=$data['bags']){
			...
			imagettftext($im, 28, 0, 40, 850, $green, $fontLight, "$batch");
			...
			if($bag==$data['bags']){
				imagettftext($im, 28, 0, 500, 850, $green, $fontLight, "SAMPLE");
			}else{
				imagettftext($im, 28, 0, 500, 850, $green, $fontLight, "$weights[$bag]".$data['unit']);
			}
