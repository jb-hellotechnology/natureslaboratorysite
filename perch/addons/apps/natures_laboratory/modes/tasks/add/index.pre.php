...
*/
    ...
    if($Form->submitted()) {
        ...
        $postvars = array('date_day', 'date_month', 'date_year', 'title', 'dueBy_day', 'dueBy_month', 'dueBy_year', 'createdBy', 'assignedTo');	   
        ...
        //FOR ITEMS PROGRAMMATICALLY ADDED TO FORM  
        $new_task = $NaturesLaboratoryTasks->create($data);

        // SHOW RELEVANT MESSAGE
        if ($new_task) {
            $message = $HTML->success_message('Task has been successfully created. Return to %sTasks%s', '<a href="'.$API->app_path().'/tasks/">', '</a>'); 
        }else{
            $message = $HTML->failure_message('Sorry, task could not be created.');
        }
